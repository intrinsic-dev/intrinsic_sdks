...
const char kUsage[] =
    "Initially moves all joints to zero rad. Plans a blended joint move, "
    "retrieves the planned trajectory for inspection and executes the motion. "
    "Eventually stops at zero joint position.";
...
absl::Status Run(const intrinsic::ConnectionParams& connection_params,
  ...
  return intrinsic::icon::examples::RunBlendedJointMove(part_name,
                                                        icon_channel);
}
}  // namespace
