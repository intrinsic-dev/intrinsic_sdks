...
// Server with registerer based Skill project and execute services.
...
int main(int argc, char** argv) {
  ...
  absl::StatusOr<intrinsic_proto::skills::SkillServiceConfig> service_config =
      intrinsic::skills::GetSkillServiceConfig(
          absl::GetFlag(FLAGS_skill_service_config_filename));
  QCHECK_OK(service_config.status())
      << "Failed to read skill service config at: "
      << absl::GetFlag(FLAGS_skill_service_config_filename);
  intrinsic::skills::RegistererSkillRepository skill_repository;
  QCHECK_OK(intrinsic::skills::SkillInit(
