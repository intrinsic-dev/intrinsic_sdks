...
class Executive:
  ...
  _last_execution_time_window: Optional[structured_logging.EventSourceWindow]
  ...
  def block_until_completed(self, *, silence_outputs: bool = False) -> None:
    ...
    if (
      ...
      if not silence_outputs:
        plan_id = self.operation.metadata.log_context.executive_plan_id
        error_summary = self.get_errors(executive_plan_id=plan_id)
  ...
  def get_errors(
      ...
      ignore_executive_plan_id: bool = False,
  ) -> error_processing.ErrorGroup:
    """Loads and optionally prints errors from current execution.

    Args:
      print_level: If True, error reports are printed.
      executive_plan_id: Optional ID of plan execution. If not specified. The ID
        of the plan currently loaded to the executive is queried.
      last_seconds_to_read: Setting for how far into the past to look for data
        about errors.
      ignore_executive_plan_id: If true, error data for all recent plans is
        loaded. This is useful if the LogItem might not contain the
        executive_plan_id in its context.

    Returns:
      Error summaries
    """
    if not executive_plan_id:
      # If the plan ID is set we know that the user only has the intention to
      # get recent entries anyway.
      print("Note that currently only recent errors can be loaded.\n")

    if not executive_plan_id and not ignore_executive_plan_id:
      executive_plan_id = self.operation.metadata.log_context.executive_plan_id
    error_summaries = self._error_loader.list_error_data(
        executive_plan_id=executive_plan_id,
        last_seconds_to_read=last_seconds_to_read,
    )
