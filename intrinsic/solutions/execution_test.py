...
class ExecutiveTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    self._structured_logs: structured_logging.StructuredLogs = (
    ...
    self._errors: error_processing.ErrorsLoader = error_processing.ErrorsLoader(
        self._structured_logs, self._installer_stub
    )
    self._executive_service_stub: executive_service_pb2_grpc.ExecutiveServiceStub = (
        mock.MagicMock()
    )
    self._blackboard_stub: blackboard_service_pb2_grpc.ExecutiveBlackboardStub = (
        mock.MagicMock()
    )
    ...
    self._pb_error: log_item_pb2.LogItem = text_format.Parse(
        """
    ...
    self._pb_error_other_plan: log_item_pb2.LogItem = text_format.Parse(
        """
        metadata { event_source: "error_report" }
        context { executive_plan_id: 1 }  # Note this ID is different
        payload { error_report {
          description {
            status: {
              code: 7
            ...
            human_readable_summary: "other text"
          }
        } }""",
        log_item_pb2.LogItem(),
    )
  ...
  def _create_operation(
      self,
      my_bt: bt.BehaviorTree = bt.BehaviorTree(root=bt.BehaviorTree.Sequence()),
  ):
    """Makes a create call to prime the executive client with an operation."""
  ...
  def test_load_works(self):
    ...
    my_bt = bt.BehaviorTree(root=bt.BehaviorTree.Sequence())
    self._executive.load(my_bt)
  ...
  def test_last_execution_time_window_works(self):
    """Tests if last_execution_time_window can be read after executive.run."""
    self._setup_create_operation()
    self._setup_start_operation()
    self._setup_get_operation_sequence([
        behavior_tree_pb2.BehaviorTree.RUNNING,
    ...
    window = self._executive.last_execution_time_window

    self.assertGreater(window.start_time, start_time)
    self.assertLess(window.end_time, end_time)
    self.assertGreater(window.end_time, window.start_time)
  ...
  def test_run_works_behavior_tree(self):
    ...
    bt_instance.set_root(
        bt.BehaviorTree.Sequence().set_children(
            bt.BehaviorTree.Task(behavior_call.Action(skill_id='action_1'))
        )
    )
  ...
  def test_errors_printed(self):
    ...
    def create_operation(state, executive_plan_id):
      operation = self._create_operation_proto(state)
      metadata = run_metadata_pb2.RunMetadata()
      operation.metadata.Unpack(metadata)
      metadata.log_context.executive_plan_id = executive_plan_id
      operation.metadata.Pack(metadata)
      return operation
    ...
    self._executive_service_stub.GetOperation.side_effect = [
        create_operation(state, 42) for state in states
    ]

    self._setup_logs_mock([self._pb_error, self._pb_error_other_plan])
  ...
  def test_get_value_works(self):
    ...
    self._blackboard_stub.GetBlackboardValue.assert_called_once_with(
        blackboard_service_pb2.GetBlackboardValueRequest(key='foo')
    )
  ...
  def test_get_value_fails_on_value_not_found(self):
    ...
    with self.assertRaises(solutions_errors.NotFoundError):
      self._executive.get_value(test_value)
    self._blackboard_stub.GetBlackboardValue.assert_called_once_with(
        blackboard_service_pb2.GetBlackboardValueRequest(key='unknown_key')
    )
  ...
  def test_is_value_available_works(self):
    ...
    response.value_infos.add(
        key='foo',
        scope=execution._PROCESS_TREE_SCOPE,
        proto_type_full_name='intrinsic_proto.test_data.TestMessage',
    )
  ...
  def test_is_value_available_by_scope_works(self):
    ...
    self._blackboard_stub.ListBlackboardValues.assert_called_once_with(
        blackboard_service_pb2.ListBlackboardValuesRequest(scope='some_scope')
    )
  ...
  def test_is_value_available_works_if_not_available(self):
    ...
    response = blackboard_service_pb2.ListBlackboardValuesResponse()
    response.value_infos.add(
        key='foo', proto_type_full_name='intrinsic_proto.test_data.TestMessage'
    )
    self._blackboard_stub.ListBlackboardValues.return_value = response
