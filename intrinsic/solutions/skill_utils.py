...
def _field_to_motion_planning_constrained_motion_target(
    field_value: worlds.ConstrainedMotionTarget,
) -> motion_target_pb2.ConstrainedMotionTarget:
  """Converts a field_value to the object world ConstrainedMotionTarget."""
  if isinstance(field_value, worlds.ConstrainedMotionTarget):
    return field_value.proto
  raise TypeError(
      f"Value: {field_value} is not convertible to a CartesianMotionTarget."
  )
...
_PYTHONIC_MESSAGE_FIELD_TYPE = {
    ...
    motion_target_pb2.ConstrainedMotionTarget.DESCRIPTOR.full_name: (
        _field_to_motion_planning_constrained_motion_target
    ),
...
def set_fields_in_msg(
  ...
  for field_name, arg_value in fields.items():
    if isinstance(arg_value, blackboard_value.BlackboardValue):
      continue
    field_type = field_descriptor_map[field_name].type
    field_message_type = field_descriptor_map[field_name].message_type
    if (
      ...
      elif field_type in _PYTHONIC_SCALAR_FIELD_TYPE and all(
          [
              isinstance(value, _PYTHONIC_SCALAR_FIELD_TYPE[field_type])
              for value in arg_value
          ]
      ):
        repeated_field.extend(arg_value)
      else:
        raise TypeError(
...
def generate_proto_infra_from_filedescriptorset(
    filedescriptor_set: descriptor_pb2.FileDescriptorSet,
) -> Tuple[descriptor_pool.DescriptorPool, Dict[str, Type[message.Message]],]:
...
def deconflict_param_and_equipment(
    equipment_slot: str,
    param_names: Container[str],
    try_suffix: str = "_equipment",
) -> str:
  """Deconflicts equipment slot name from existing parameters.

  Equipment slots and parameter names are two separate namespaces in the skill
  specification. But for our API purposes, they both become parameters to the
  same function, and thus cannot be the same.

  Args:
    equipment_slot: equipment slot name
    param_names: container with all regular parameter names
    try_suffix: suffix to append on conflict to try deconfliction

  Returns:
    This function checks whether the equipment_slot is in param_names. If it is
    not, the equipment_slot is returned unmodified. If it is contained, the
    try_suffix is added. If it still is contained, an exception is raised.
    Otherwise the extended name is returned.

  Raises:
    NameError: if equipment_slot as well as equipment_slot+try_suffix is already
      contained in param_names
  """
  if equipment_slot not in param_names:
    return equipment_slot

  # We have a conflict, a skill has an equipment slot and a parameter with
  # the same name (we are mixing two namespaces here). Add suffix to
  # slot name to disambiguate
  try_slot = equipment_slot + try_suffix
  if try_slot in param_names:
    # Still a conflict!? Ok, out of luck, we cannot recover that one
    raise NameError(
        f"'{equipment_slot}' and '{try_slot}' are both parameter "
        "names and equipment slots."
    )
  return try_slot
...
def extract_parameter_information_from_message(
  ...
  for field in param_defaults.DESCRIPTOR.fields:
    ...
    if field.label == descriptor.FieldDescriptor.LABEL_REPEATED:
      field_type = repeated_pythonic_field_type(field, field_name_to_type_name)
