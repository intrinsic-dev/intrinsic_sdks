...
class DeploymentsTest(absltest.TestCase):
  ...
  def test_connect_raises_on_invalid_params(self):
    with self.assertRaisesRegex(ValueError, "project.*solution"):
      deployments.connect(project="test-project")
  ...
  def test_connect_project_and_solution(
      self,
    ...
    deployments.connect(project="test-project", solution="test-solution")
...
class SolutionTest(absltest.TestCase):
  ...
  def setUp(self):
    ...
    structured_logs_stub = mock.MagicMock()
    structured_logs = structured_logging.StructuredLogs(structured_logs_stub)

    self._installer_stub = mock.MagicMock()
    errors = error_processing.ErrorsLoader(
        structured_logs, self._installer_stub
    )
...
This skill requires no equipment
