...
# Typing aliases
# Maps slot name to equipment names.
EquipmentMap = Dict[str, equipment_mod.EquipmentHandle]
...
def _gen_init_docstring(
    ...
    compatible_equipment: Dict[str, equipment_mod.EquipmentList],
) -> str:
  """Generates documentation string for init function.

  Args:
    info: Skill information.
    compatible_equipment: Map from equipment slot names to equipment suitable
      for that slot. It is used to determine whether a default value can be
      assigned for equipment parameters.

  Returns:
    Python documentation string.

  Raises:
    NameError: a parameter name and equipment name are the same and even
      disambiguation adding a "_equipment" suffix failed.
  """
  ...
  for slot, selector in sorted(info.skill_proto.equipment_selectors.items()):
    param_name = skill_utils.deconflict_param_and_equipment(slot, param_names)

    slot_docstring = []
    if len(selector.equipment_type_names) == 1:
      slot_docstring.append(
          f"equipment of type {selector.equipment_type_names[0]}"
      )
    else:
      slot_docstring.append(
          "Equipment implementing all of the following types:"
      )
      for t in selector.equipment_type_names:
        slot_docstring.append(f"  - {t}")

    slot_compatible_equipment = compatible_equipment[param_name]
    if len(slot_compatible_equipment) == 1:
      default_equipment = slot_compatible_equipment[
          dir(slot_compatible_equipment)[0]
      ]
      slot_docstring.append(f"Default equipment: {default_equipment.name}")
  ...
  if not info.skill_proto.equipment_selectors:
    docstring.append("This skill requires no equipment")
...
def _gen_init_params(
  ...
  # Add equipment slot names, if an equipment slot has the same name as
  # a skill parameter (actually different namespaces) disambiguate by suffixing
  # the slot name with "_equipment".
  for slot in sorted(info.skill_proto.equipment_selectors.keys()):
    default = inspect.Parameter.empty
    if slot in compatible_equipment:
      slot_compatible_equipment = compatible_equipment[slot]
      if len(slot_compatible_equipment) == 1:
        # An equipment might be contained twice for the slot in
        # compatible_equipment if the name had to be mangled to be compatible as
        # a Python attribute name. Using dir() will filter this for us, so just
        # take the first and only of these entries to assign.
        default = slot_compatible_equipment[dir(slot_compatible_equipment)[0]]
    params.append(
        inspect.Parameter(
            skill_utils.deconflict_param_and_equipment(slot, param_names),
            inspect.Parameter.KEYWORD_ONLY,
            annotation=equipment_mod.EquipmentHandle,
            default=default,
        )
    )
...
def _gen_init_fun(
  ...
  ] + _gen_init_params(info, compatible_equipment, nested_classes)
  ...
  new_init_fun.__doc__ = _gen_init_docstring(info, compatible_equipment)
...
def _gen_skill_class(
  ...
  Args:
    info: Skill information.
    compatible_equipment: Map with compatible equipment.
  ...
  skill_methods = {
      "_info": info,
      "_compatible_equipment": providers.SkillCompatibleEquipmentMap(
          compatible_equipment
      ),
      # We use the __init__ documentation because that is shown in the
      # auto-completion tooltip, not __init__.__doc__.
      "__doc__": _gen_init_docstring(info, compatible_equipment),
  }
  ...
  init_fun = _gen_init_fun(info, compatible_equipment, nested_classes)
...
class GeneratedSkill(providers.SkillBase):
  ...
  _compatible_equipment: Dict[str, equipment_mod.EquipmentList] = {}
  ...
  def _set_equipment(self, **kwargs) -> List[str]:
    """Set equipment requirements of skill.
    ...
    for slot, selector in self._info.skill_proto.equipment_selectors.items():
      slot_param_name = slot
      if slot in self._info.field_names:
        slot_param_name = slot + "_equipment"
      compatible_equipment = self.compatible_equipment[slot_param_name]
      if slot_param_name not in kwargs and len(compatible_equipment) != 1:
        if not compatible_equipment:
          raise KeyError(
              f"Equipment argument '{slot_param_name}' is missing and "
              "no compatible equipment has been configured for this "
              "workcell."
          )
        raise KeyError(
            f"Equipment argument '{slot_param_name}' is missing. "
            "There is more than one compatible equipment ("
            f"{', '.join([e.name for e in compatible_equipment])})."
        )

      equipment = (
          kwargs[slot_param_name]
          if slot_param_name in kwargs and kwargs[slot_param_name] is not None
          else compatible_equipment[dir(compatible_equipment)[0]]
      )

      if not isinstance(equipment, equipment_mod.EquipmentHandle):
        raise TypeError(
            f"Given value for equipment slot '{slot_param_name}' "
            "is not an EquipmentHandle."
        )

      given_types = set(equipment.types)
      expected_types = set(selector.equipment_type_names)

      if not expected_types.issubset(given_types):
        raise TypeError(
            f"Expected types ({', '.join(expected_types)}) "
            f"for slot {slot}, but given equipment handle has "
            f"({', '.join(given_types)})"
        )

      self._equipment[slot] = equipment
      equipment_set.append(slot_param_name)
  ...
  def proto(self) -> behavior_call_pb2.BehaviorCall:
    ...
    for slot, handle in self._equipment.items():
    ...
    if self.proto.equipment:
      for key, value in sorted(self.proto.equipment.items()):
        ...
        if key in self._info.field_names:
          slot_param_name = key + "_equipment"
        equipment.append(
            "{}={{{}}}".format(slot_param_name, repr(value).strip())
        )
    return (
        f"skills.{_skill_name_from_id(self.proto.skill_id)}({', '.join(params+equipment)})"
    )

  @classproperty
  def compatible_equipment(cls) -> providers.SkillCompatibleEquipmentMap:  # pylint:disable=no-self-argument
    return cls._compatible_equipment
...
class Skills(providers.SkillProvider):
  ...
  _compatible_equipment_by_name: Dict[
      str, Dict[str, equipment_mod.EquipmentList]
  ]
  _compatible_equipment_by_id: Dict[str, Dict[str, equipment_mod.EquipmentList]]
  ...
  def __init__(
    ...
    self._compatible_equipment_by_name = {}
    self._compatible_equipment_by_id = {}
    self.update()
  ...
  def update(self) -> None:
    ...
    for skill_info in self._skills_by_name.values():
      selectors.extend(skill_info.skill_proto.equipment_selectors.values())
    handles_by_selector = self._equipment_registry.find_equipments(selectors)

    # Update compatible equipment for skills
    selector_index = 0
    for skill_name, skill_info in self._skills_by_name.items():
      self._compatible_equipment_by_name[skill_name] = {}
      self._compatible_equipment_by_id[skill_info.id] = {}
      for slot in skill_info.skill_proto.equipment_selectors:
        handles = handles_by_selector[selector_index]
        equipment_slot = slot
        if equipment_slot in skill_info.field_names:
          equipment_slot = slot + "_equipment"
        self._compatible_equipment_by_name[skill_name][equipment_slot] = (
            equipment_mod.EquipmentList(
                [equipment_mod.EquipmentHandle(h) for h in handles]
            )
        )
        self._compatible_equipment_by_id[skill_info.id][equipment_slot] = (
            self._compatible_equipment_by_name[skill_name][equipment_slot]
        )
        selector_index += 1
  ...
  def __getattr__(self, name: str) -> Type[Any]:
    ...
    if name not in self._skill_type_classes_by_name:
      self._skill_type_classes_by_name[name] = _gen_skill_class(
          self._skills_by_name[name], self._compatible_equipment_by_name[name]
      )
    ...
    if name in self._skills_by_name:
      ...
      if name not in self._skill_type_classes_by_id:
        self._skill_type_classes_by_id[name] = _gen_skill_class(
            self._skills_by_id[name], self._compatible_equipment_by_id[name]
        )
