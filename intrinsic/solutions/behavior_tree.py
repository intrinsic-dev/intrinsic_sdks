...
import enum
import sys
from typing import Any as AnyType, Iterable, List, Optional, Sequence as SequenceType, Tuple, Union
import uuid
...
class Decorators:
  ...
  def print_python_code(
    ...
    if self.condition is not None or self.breakpoint_type is not None:
      identifier = _generate_unique_identifier('decorator', identifiers)
      sys.stdout.write(f'{identifier} = BT.Decorators(')
      if self.condition is not None:
        sys.stdout.write(f'condition={condition_identifier}')

      if self.breakpoint_type is not None:
        if self.condition is not None:
          sys.stdout.write(', ')
        sys.stdout.write(f'breakpoint_type=BT.{self.breakpoint_type}')
...
class SubTreeCondition(Condition):
  ...
  Attributes:
    proto: The proto representation of the node.
    condition_type: A string label of the condition type.
    node: The subtree deciding the outcome of the condition.
  """

  def __init__(self, node: Union['Node', actions.ActionBase]):
    node = _transform_to_optional_node(node)
    if node is None:
  ...
  def __repr__(self) -> str:
    """Returns a compact, human-readable string representation."""
    return f'{type(self).__name__}({str(self.node)})'

  @property
  def proto(self) -> behavior_tree_pb2.BehaviorTree.Condition:
    proto_object = behavior_tree_pb2.BehaviorTree.Condition()
    proto_object.tree.CopyFrom(self.node.proto)
    return proto_object
  ...
  def create_from_proto(
    ...
    if not proto_object.HasField('tree'):
      raise ValueError('Received invalid condition proto.')

    return cls(Node.create_from_proto(proto_object.tree))

  def print_python_code(
      self, identifiers: List[str], skills: providers.SkillProvider
  ) -> str:
    tree_identifier = self.node.print_python_code(identifiers, skills)
    identifier = _generate_unique_identifier('subtree_condition', identifiers)
    print(f'{identifier} = BT.SubTreeCondition(node={tree_identifier})')
    return identifier
...
class Task(Node):
  ...
  def print_python_code(
    ...
    task_identifier = _generate_unique_identifier(name, identifiers)
    print(
...
class BehaviorTree:
  ...
  # TypeAliases for easy access in Jupyter
  WorldQuery = WorldQuery
  BreakpointType = BreakpointType
  Decorators = Decorators
  Node = Node
  Condition = Condition
  SubTreeCondition = SubTreeCondition
  ...
  def print_python_code(self, skills: providers.SkillProvider) -> None:
    identifiers = []
    ...
    print(f"{identifier} = BT(name='{self.name}', root={child_identifier})")
