...
class Rotation3Test(parameterized.TestCase, math_test.TestCase):
  ...
  def test_axis_angle(self, axis, angle):
    ...
    axis_out, angle_out = rotation.axis_angle()
  ...
  def test_axis_angle_default_vector(self, rotation):
    ...
    for default_name, default_axis in _TEST_UNIT_VECTORS:
      with self.subTest(default_name=default_name, default_axis=default_axis):
        axis_with_default, angle_with_default = rotation.axis_angle(
  ...
  def test_axis_angle_direction_vector(self, rotation):
    ...
    for direction_name, direction_axis in _TEST_UNIT_VECTORS:
      with self.subTest(
        ...
        axis_with_direction, angle_with_direction = rotation.axis_angle(
  ...
  def check_tiny_rotation(self, rotation_tiny_angle, expected_angle):
    ...
    axis_out, angle_out = rotation_tiny_angle.axis_angle()
  ...
  def test_assert_rotation_close(self, rotation):
    ...
    rotation_i = rotation3.Rotation3(quaternion.Quaternion.i())
    self.assertRaisesRegex(
        AssertionError,
  ...
  def test_matrix3x3(self, axis, angle):
    ...
    q_left = rotation.quaternion.left_multiplication_matrix()
    qi_right = rotation.quaternion.inverse().right_multiplication_matrix()
    logging.debug(
        'q: %r\nq_left: %r\nqi_right: %r\nmatrix: %r\n%r\n%r',
        rotation.quaternion,
        q_left,
    ...
    self.assert_all_close(rotation_matrix, np.matmul(q_left, qi_right)[:3, :3])
    self.assert_all_close(rotation_matrix, np.matmul(qi_right, q_left)[:3, :3])
  ...
  def test_euler_angles_errors(self):
    self.assertRaisesRegex(
        ValueError,
        rotation3.MATRIX_NOT_ORTHOGONAL_MESSAGE,
        rotation3.euler_angles_from_matrix,
        np.ones((3, 3)),
    )
  ...
  def test_euler_angles_correctness(self, rotation):
    ...
    roll_axis, roll_out = rotation_roll.axis_angle(
    ...
    pitch_axis, pitch_out = rotation_pitch.axis_angle(
    ...
    yaw_axis, yaw_out = rotation_yaw.axis_angle(
  ...
  def test_ne(self, rotation):
    self.check_unequal_rotations(
        rotation, rotation * rotation3.Rotation3(quaternion.Quaternion.i())
    )
    self.check_unequal_rotations(
  ...
  @parameterized.named_parameters(_TEST_UNIT_VECTORS)
  def test_rotation_between(self, src):
    for target_name, target in _TEST_UNIT_VECTORS:
      with self.subTest(target_name=target_name, target=target):
        rotation_matrix = rotation3.matrix_rotation_between(
        ...
        logging.debug(
            ...
            rotation_matrix,
            src,
            np.matmul(rotation_matrix, src),
            target,
        )
        src_to_target = rotation3.Rotation3.rotation_between(
            src=src, target=target
        )
        logging.debug(
            '%r * %r = %r == %r',
            src_to_target,
            src,
            src_to_target.rotate_point(src),
            target,
        )
        self.assert_all_close(src_to_target.rotate_point(src), target)
  ...
  @parameterized.named_parameters(_TEST_UNIT_VECTORS)
  def test_rotation_between_tiny_angle(self, src):
    for target_name, delta_target in _TEST_UNIT_VECTORS:
      with self.subTest(target_name=target_name, delta_target=delta_target):
        target = src + _LESS_THAN_EPSILON * delta_target
        src_to_target = rotation3.Rotation3.rotation_between(
            src=src, target=target
        )
        logging.debug(
  ...
  @parameterized.parameters(
      (quaternion.Quaternion.one(), _quaternion_proto('w: 1')),
      (quaternion.Quaternion.i(), _quaternion_proto('x: 1')),
      (quaternion.Quaternion.j(), _quaternion_proto('y: 1')),
      (quaternion.Quaternion.k(), _quaternion_proto('z: 1')),
      (
          quaternion.Quaternion(np.full(4, 0.5)),
          _quaternion_proto('x: 0.5 y: 0.5 z: 0.5 w: 0.5'),
      ),
      (
          quaternion.Quaternion(np.full(4, 0.5)),
          _quaternion_proto('x: 1 y: 1 z: 1 w: 1'),
      ),
      (
      ...
      (
          quaternion.Quaternion([-0.4, 0.6, -0.8, 1.0]),
          _quaternion_proto('x: -0.4 y: 0.6 z: -0.8 w: 1.0'),
      ),
  )
  def test_to_and_from_proto(self, quat, quat_proto):
    rotation = rotation3.Rotation3(quat)
    self.assertEqual(rotation, rotation3.Rotation3.from_proto(quat_proto))
    self.assertEqual(
        rotation, rotation3.Rotation3.from_proto(rotation.to_proto())
    )
    proto_out = quaternion_pb2.Quaterniond()
    rotation.to_proto(proto_out)
    self.assertEqual(rotation, rotation3.Rotation3.from_proto(proto_out))
    self.assertEqual(id(proto_out), id(rotation.to_proto(proto_out)))
    self.assertNotEqual(id(proto_out), id(rotation.to_proto()))

  def test_to_and_from_empty_proto(self):
    quat_proto = quaternion_pb2.Quaterniond()
    quat = quaternion.Quaternion.from_proto(quat_proto)
    rotation = rotation3.Rotation3.from_proto(quat_proto)
    self.assertEqual(quat, quaternion.Quaternion.zero())
    self.assertEqual(rotation, rotation3.Rotation3.identity())
    self.assertEqual(
        rotation, rotation3.Rotation3.from_proto(rotation.to_proto())
    )

  @parameterized.parameters(
      _quaternion_proto('x: 1e-12'),
      _quaternion_proto('y: -1e-9'),
      _quaternion_proto('z: 1e-10'),
      _quaternion_proto('w: -1e-11'),
      _quaternion_proto('x: 1e-12 y: -1e-13 z: 1e-14 w: -1e-15'),
  )
  def test_from_proto_error(self, quat_proto):
    self.assertRaisesRegex(
        ValueError,
        quaternion.QUATERNION_ZERO_MESSAGE,
        rotation3.Rotation3.from_proto,
        quat_proto,
    )
