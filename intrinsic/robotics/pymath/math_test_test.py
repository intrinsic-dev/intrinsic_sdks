...
"""Tests for intrinsic.robotics.pymath.math_test module."""
...
class MathTest(math_test.TestCase, parameterized.TestCase):
  ...
  def test_make_named_nonunit_quaternions(self):
    ...
    for name, nonunit_quaternion in named_nonunit_quaternions:
      with self.subTest(name=name, nonunit_quaternion=nonunit_quaternion):
        self.assertGreater(nonunit_quaternion.norm(), 1)
  ...
  def test_make_named_tiny_quaternions(self):
    ...
    for name, tiny_quaternion in named_tiny_quaternions:
      with self.subTest(name=name, tiny_quaternion=tiny_quaternion):
        self.assertLessEqual(
            tiny_quaternion.norm(), math_test._LESS_THAN_EPSILON * 2.0
        )
  ...
  def test_assert_value_in_interval(self):
    self.assert_value_in_interval(0, [0, 0])
    self.assert_value_in_interval(0, [0, 1])
    self.assert_value_in_interval(0, [-1, 0])
    self.assert_value_in_interval(0, [-1, 1])

  def test_assert_value_in_interval_fail(self):
    self.assertRaisesRegex(
        AssertionError,
        math_test.ASSERT_CONTAINS_MESSAGE,
        self.assert_value_in_interval,
        0,
        [1, 2],
    )
    self.assertRaisesRegex(
