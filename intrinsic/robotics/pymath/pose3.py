...
class Pose3(object):
  ...
  def quaternion(self) -> quaternion_class.Quaternion:
    ...
    Returns:
      A unit quaternion.
    """
    return self.rotation.quaternion
  ...
  def transform_point_by_inverse(
      self, point: math_types.VectorType
  ) -> np.ndarray:
    """Transforms the point by the inverse of this pose.

      T(v) = q v q' + p
      T'(v) = q' (v - p) q
      (T o T')(v) == v

    If we have a transform, world_pose_A, and a point, point_world, in the world
    coordinate frame, we can calculate the point in coordinate frame A as:

      point_A = world_pose_A.transform_point_by_inverse(point_world)

    This is more stable than computing the inverse and then transforming the
    point, because the translation component of the pose is subtracted directly
    from the point without rotation.
    ...
    Returns:
     A 3D vector in a numpy array.
    """
    # Performs rotation by the conjugate of the unit quaternion.
    #   q * v * q' rotates the point by q.
    #   q' * v * q'' = q' v q rotates the point by q'.
    point_transformed = self.rotation.inverse().rotate_point(
  ...
  def to_proto(
      self, proto_out: Optional[pose_pb2.Pose3d] = None
  ) -> pose_pb2.Pose3d:
    """Populates a Pose3d protobuf with this pose.

    Args:
      proto_out: The output protobuf.  If not specified, create a new one.

    Returns:
      A Pose3d protobuf describing this pose. Returns the input protobuf if
      specified.
    """
    if proto_out is None:
      proto_out = pose_pb2.Pose3d()
    self.rotation.to_proto(proto_out.rotation)
    proto_out.translation.x = self.translation[0]
    proto_out.translation.y = self.translation[1]
    proto_out.translation.z = self.translation[2]
    return proto_out
  ...
  def check_valid(
    ...
    """Checks that the pose has valid values.
    ...
    """
    return self.rotation.check_valid(norm_epsilon, err_msg)
  ...
  @classmethod
  def from_proto(cls, pose3_proto: pose_pb2.Pose3d) -> 'Pose3':
    """Constructs a Pose3 from a Pose3d protobuf message.

    Uses Rotation3.from_proto to interpret the quaternion rotation component of
    the pose.

    Args:
      pose3_proto: A Pose3d protobuf message.

    Returns:
      The pose represented by the protobuf.

    Raises:
      ValueError: If the rotation is invalid.
    """
    rotation = rotation3.Rotation3.from_proto(pose3_proto.rotation)
    translation = [
        pose3_proto.translation.x,
        pose3_proto.translation.y,
        pose3_proto.translation.z,
    ]
    return cls(rotation=rotation, translation=translation)
  ...
  def from_lookat(
      cls,
      eye: math_types.Vector3Type,
      target: math_types.Vector3Type,
      up: math_types.Vector3Type,
  ) -> 'Pose3':
    """Constructs a camera pose, world_pose_camera, that positions a camera with OpenGL conventions.

    The pose transforms
      the camera origin to the eye,
      the negative z-axis to a ray from the eye through the target,
      and the positive y-axis as close as possible to the up vector.

    Args:
      eye: Place of the camera.
      target: Target to look at from the camera.
      up: Up vector for the camera.

    Returns:
      world_pose_camera as explained above.

    Raises:
      ValueError if `target-eye` is parallel to `up`.
    """
    eye = vector_util.as_vector(eye)
    target = vector_util.as_vector(target)
    up = vector_util.as_vector(up)
    ...
    epsilon = 1.0e-9
    if np.all(np.abs(right) <= epsilon):
      raise ValueError(
          'Specified input is degenerated, i.e. the look direction and up are '
          'parallel: look_dir=%s, up=%s' % (-back, up)
      )
    ...
    m[:-1, 3] = eye
    m[3, :] = [0, 0, 0, 1]
    return Pose3.from_matrix4x4(m)

  @classmethod
  def from_lookat_with_auto_up(
      cls, eye: math_types.Vector3Type, target: math_types.Vector3Type
  ) -> 'Pose3':
    """Constructs a camera pose, world_pose_camera, that positions a camera with OpenGL conventions.

    The pose transforms the camera origin to the eye, the negative z-axis to a
      ray from the eye through the target, and the positive y-axis to point up.
      up in this case is determined automatically, by first trying (0, 0, 1),
      and if that doesn't work (because `target-eye` is parallel to z) it falls
      back to (0, 1, 0). The final up vector uses this temporary values and
      eventually is made orthogonal on the place defined by the look direction
      and the side vector.
    ...
    epsilon = 1.0e-9
    if np.all(np.abs(right) <= epsilon):
      # If forward is parallel to up, the right vector is going to be
      # degenerated. To avoid debugging sessions later on, choose another vector
      # as the up vector.
      # Try the y axis as up vector instead of the given one.
      up = [0, 1, 0]
      right = np.cross(up, back)
    right = right / np.linalg.norm(right)

    up = np.cross(back, right)
    return Pose3.from_lookat(eye, target, up)
