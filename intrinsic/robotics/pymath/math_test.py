...
"""robotics.pymath.TestCase - Base class for tests in robotics/pymath (python3).
...
from intrinsic.robotics.pymath import pose3
...
INVALID_INTERVAL_MESSAGE = 'Interval must be defined by two values.'
...
def make_named_unit_quaternions() -> List[NamedQuaternionType]:
  ...
  return [
      ...
      ('i', quaternion.Quaternion.i()),
      ('j', quaternion.Quaternion.j()),
      ('k', quaternion.Quaternion.k()),
...
def make_named_tiny_quaternions() -> List[NamedQuaternionType]:
  ...
  named_quaternions = [('0', quaternion.Quaternion.zero())]
  named_quaternions += [
...
class TestCase(absltest.TestCase):
  ...
  def assert_value_in_interval(
      self, value: float, ival_bounds: math_types.VectorType, err_msg: Text = ''
  ) -> None:
    """Asserts that the value lies in the interval.

      ival[0] <= value <= ival[1]

    Args:
      value: A scalar value.
      ival_bounds: An interval defined by min and max bounds as a tuple or list
        of two values.
      err_msg: Error message displayed if the assertion fails.

    Raises:
      AssertionError: If the interval is improperly defined or if the value is
        not contained in the interval.
    """
    ival = interval.Interval(bounds=ival_bounds)
    self.assert_contains(ival, value, err_msg=err_msg)
  ...
  def assert_interval_close(
    ...
    """Asserts that the two intervals are approximately equivalent.

    If the assertion fails, it will result in an error message that matches
    the pattern in INTERVAL_NOT_CLOSE_MESSAGE.

    Args:
      interval1: First interval in comparison.
      interval2: Second interval in comparison.
      rtol: relative error tolerance, passed through to np.allclose.
      atol: absolute error tolerance, passed through to np.allclose.
      err_msg: Error message displayed if the assertion fails.

    Raises:
      AssertionError: If the two intervals are not equivalent within epsilon.
    """
    self.assertTrue(
        interval1.almost_equal(interval2, rtol=rtol, atol=atol),
        '%s: %r =! %r with rtol=%g, atol=%g %s'
        % (
            INTERVAL_NOT_CLOSE_MESSAGE,
            interval1,
            interval2,
            rtol,
            atol,
