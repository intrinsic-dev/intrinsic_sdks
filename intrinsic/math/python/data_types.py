  ...
  def __init__(
    ...
    if linear is None:
      ...
      self.linear = vector_util.as_vector3(
          linear, dtype=np.float64, err_msg='Linear vector in Twist'
      ).copy()
    if angular is None:
      self.angular = np.zeros(3)
    else:
      self.angular = vector_util.as_vector3(
          angular, dtype=np.float64, err_msg='Angular vector in Twist'
      ).copy()
  ...
  def __init__(
    ...
    if force is None:
      ...
      self.force = vector_util.as_vector3(
          force, dtype=np.float64, err_msg='Force vector in Wrench'
      ).copy()
    if torque is None:
      self.torque = np.zeros(3)
    else:
      self.torque = vector_util.as_vector3(
          torque, dtype=np.float64, err_msg='Torque vector in Wrench'
      ).copy()
  ...
  def __init__(
    ...
    if matrix6x6 is not None:
      ...
      if linear is None:
        ...
        self.linear = vector_util.as_vector3(
            linear, dtype=np.float64, err_msg='Linear vector in Stiffness'
        ).copy()
      ...
      if torsional is None:
        ...
        self.torsional = vector_util.as_vector3(
            torsional, dtype=np.float64, err_msg='Angular vector in Stiffness'
        ).copy()
...
def pose3_to_transform(pose: Pose3) -> cart_space_pb2.Transform:
  """Converts a pose3 to an intrinsic_proto.Transform.

  Args:
    pose: A Pose3 object.

  Returns:
    A Transform proto object.
  """
  return cart_space_pb2.Transform(
      pos=cart_space_pb2.Point(x=pose.vec7[0], y=pose.vec7[1], z=pose.vec7[2]),
      rot=cart_space_pb2.Rotation(
          qx=pose.vec7[3], qy=pose.vec7[4], qz=pose.vec7[5], qw=pose.vec7[6]
      ),
  )


def transform_to_pose3(transform: cart_space_pb2.Transform) -> Pose3:
  """Converts an intrinsic_proto.Transform to a pose3.

  Args:
    transform: A Transform proto object.

  Returns:
    A Pose3 object.
  """
  return Pose3.from_vec7([
      transform.pos.x,
      transform.pos.y,
      transform.pos.z,
      transform.rot.qx,
